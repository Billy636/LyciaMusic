...
本节复盘的是报告形成后实际完成的提交。结论分为两层：

- **代码路径层面可以确认已优化。** 常驻实例、缓存权重上限、扫描克隆、不可见频谱计算、后台唤醒、临时排序等工作量或对象上界已经降低，相关测试和查询计划能够证明新路径生效。
- **尚不能把具体 MB 或 CPU 百分比写成最终事实。** 当前验证包含自动化测试、生产前端构建、本地交互回归和 SQLite 合成基准，但还没有完成第 8 节要求的 Release 进程组 A/B 采样。因此可以确认方向和机制有效，不能声称整机 Private WS 已下降某个固定比例。

| 小节 | 提交 | 实施结果 | 性能占用判断 | 用户体验保护与验证 |
|---|---|---|---|---|
| 5.1 多窗口入口 | `49e9f56` | 按窗口 label 动态加载独立根组件；辅助窗口不再静态触达主壳和 router | 确认减少辅助 WebView 的模块解析和无关运行时对象；当前构建已形成独立窗口 chunk | 窗口协议和 UI 不变；保留托盘/mini 预热；bootstrap 依赖边界有自动化测试 |
| 5.2 路径缓存 | `a88fa50` | 四类路径缓存合并为共享加权 LRU；正常预算 16 MiB、低功耗 8 MiB；当前项受保护；统计包含 weight 和 weightEvictions | 确认把“大数组只按条目计数”改成字节权重上限，消除大曲库缓存体积事实上无界的问题 | 当前视图不因后台预算被淘汰；TTL/LRU/低功耗收缩和权重淘汰测试通过 |
| 5.3 频谱链路 | `4dd69cb` | 后端原子启停、采样 buffer/FFT 结果复用；相同 cursor 不重复分析；前端 30 FPS、渐变和尺寸缓存 | 详情关闭或低功耗时可视化采样写入与 FFT 为 0；详情打开时短命分配和重复绘制显著减少 | 频段、柱数、音频处理链和 30 FPS 目标不变；暂停、隐藏、恢复路径均有测试 |
| 5.4 远程预缓存 watcher | 未实施 | 当前仍在 60% 之后随 `currentTime` 更新执行 `queuePaths.indexOf`，仅靠相同 nextPath 防止重复网络预缓存 | **仍是明确的 CPU 热路径，尤其是超大播放队列** | 行为当前未改变；应作为下一项低风险优化 |
| 5.5 扫描峰值 | `b198b40` | worker 改为半数逻辑核心且最多 8；差异阶段以 change kind + index 转移所有权；常规扫描使用摘要结果并及时释放完整歌曲 | 确认降低并行 CPU 峰值、完整 `Song` 克隆数量和无用全量结果驻留；扫描总结果语义保持一致 | scanner 定向测试和完整 Rust 测试通过；未改变 CUE、过滤、删除和进度语义 |
| 5.6 辅助窗口生命周期 | `5cbe279` | 任务栏设置关闭时 destroy；全屏临时遮挡仍 hide；mini 隐藏后保留 2 分钟热租约再销毁 | 确认长期不用时可释放 WebView，同时避免频繁开关反复冷启动 | 热租约内再次打开沿用原窗口；销毁后 ready 状态可重建；生命周期测试通过 |
| 5.7 重型 UI | `990585b` | PlayerDetail、播放队列和两个弹窗按需挂载；退场动画结束后卸载；歌词页、AMLL 和轻量歌词分别动态加载 | 确认未打开时不再常驻 DOM、监听器、Canvas/AMLL 实例；构建产物中 PlayerDetail、LyricsView、AmlLyricPlayer、LightLyricPlayer 已拆成独立 chunk | 本地实际打开/关闭详情页和队列无空白帧，退场动画完整；全量前端测试通过 |
| 5.8 后台唤醒 | `c4df875` | 任务栏全屏检测保持 1 秒，几何校准降为 10 秒兜底并保留 DPI 事件；Rust 播放器仅空闲时从 150ms 退避到 1 秒；进度持久化从 2 秒改为 5 秒 | 空闲播放器线程理论唤醒由约 6.7 次/秒降到 1 次/秒；任务栏重型几何查询降低约 90%；localStorage 周期检查降低 60% | 播放中仍为 150ms；命令通道会立即唤醒；全屏响应仍为 1 秒；暂停、切歌、隐藏和退出立即持久化，恢复误差不超过 5 秒 |
| 5.9 SQLite/搜索 | `30bf89c` | 搜索输入使用 150ms 提交 debounce，清空立即生效；默认标题排序增加完全匹配 ORDER BY 的表达式索引；新增 50k/100k 基准和查询计划断言 | 100k 合成曲库标题分页 P95 约从无索引 45.1ms 降至 4.6ms；50k 约从 11.7ms 降至 2.1ms；避免临时排序 B-tree | 输入文字逐字显示，只有查询提交延后 150ms；清空无延迟；排序语义不变；未盲目调整 SQLite 临时表策略 |

### 11.2 5.9 基准记录与 SQLite 决策

命令：

```text
cargo test --manifest-path src-tauri/Cargo.toml benchmark_large_library_search_and_sort_memory -- --ignored --nocapture
```

同一次 Debug 合成基准中的结果如下。绝对耗时不能代替 Release 数据，但同进程、同数据下的有索引/强制无索引对比可以证明索引收益：

| 曲库规模 | 标题分页 P95（索引） | 标题分页 P95（强制无索引） | 包含式搜索 P95 | 查询后 SQLite current/high-water |
|---:|---:|---:|---:|---:|
| 50k | 2.09ms | 11.66ms | 110.04ms | 14,206,024 / 14,253,016 bytes |
| 100k | 4.58ms | 45.13ms | 349.09ms | 28,452,264 / 28,499,256 bytes |

查询阶段的 SQLite high-water 增量约为 47KB，而不是随排序产生数十 MB 的额外临时内存；`EXPLAIN QUERY PLAN` 也确认默认标题排序使用 `idx_songs_all_view_title` 且没有 `USE TEMP B-TREE`。因此本轮保留 `temp_store=MEMORY`：改成磁盘临时表没有足够内存收益证据，却可能引入机械硬盘或低速磁盘上的可感知延迟。

100k 包含式搜索 P95 仍约 349ms，说明 `LOWER(...) LIKE '%query%'` 的全表扫描仍有进一步优化价值。150ms debounce 能显著减少连续输入期间的无效请求，但不会加速最终一次查询；只有在真实 100k 曲库 Release 基准也确认这是热点后，才应引入 FTS5。

### 11.3 是否真的降低了占用且没有影响体验

综合判断为：**是，已实施部分确实减少了内存上界、持续 CPU 工作和峰值工作量；当前没有发现正常体验回退，但仍需 Release A/B 才能量化整机收益。**

支持这一判断的直接证据：

1. 多窗口和重型 UI 的依赖/实例边界已实际拆开，不可见组件不再提前 mount；这不是仅压缩文件，而是减少独立 WebView 中的运行时对象。
2. 路径缓存有明确的 16 MiB/8 MiB 权重预算，扫描 worker、克隆数量和返回对象都有确定上界；这些改动直接约束内存，而不是依赖 GC 偶然回收。
3. 频谱不可见时后端开关为 false，空闲播放器和任务栏几何查询的唤醒次数可由常量直接推导下降。
4. SQLite 默认排序的查询计划和 50k/100k 相对基准证明 CPU/临时排序工作已减少。
5. 音质、WASAPI 路径、歌词时间精度、频谱规格、列表虚拟化、全屏检测频率和窗口动画均未降低；本地交互回归未出现详情页空白帧或队列退场丢失。

仍需保留的审慎结论：ES module 下载后通常仍在模块缓存中，所以 5.7 主要释放的是实例、DOM、观察器和 Canvas，而不是保证 chunk 字节从进程完全消失；mini 销毁后的 WebView2 工作集回落也需要按进程组观察，不能只看 Rust 主进程。

### 11.4 进一步优化空间（按建议优先级）

1. **完成 5.4 远程预缓存阈值状态机。** 在歌曲/队列变化时计算一次 next path，只在 60% 布尔阈值跃迁时触发，消除播放 tick 上的 O(N) `indexOf`；这是当前最明确、风险最低的遗漏项。
2. **补齐 Release 进程组 A/B。** 按第 8 节对首页静置、普通播放、详情频谱、隐藏托盘、mini 租约、任务栏开关和扫描进行 3～5 次冷/暖测量；记录 Private WS、Private Bytes、CPU、WebView 数和 P95，形成可以对外声明的量化结论。
3. **FTS5 先做旁路原型。** 真实 100k Release 搜索若仍超过体验门槛，再建立 title/artist/album/path FTS 表、同步写入/删除和回退查询；必须验证中文、日文、路径子串和多歌手语义，不能直接替换现有 LIKE。
4. **任务栏前台事件化。** 使用 Win32 foreground/display/taskbar 事件即时触发，保留 5～10 秒兜底；当前实现为了体验仍保留 1 秒轻量全屏查询。
5. **扫描继续演进为有界 pipeline。** 当前已去除主要克隆并限制 worker，但尚未按前台播放/后台空闲动态提升或降低 QoS，也未把收集、解析、写库全部改成 128～512 首的有界批次。
6. **用遥测调校缓存预算和窗口租约。** 观察 weightEvictions、命中率、mini 冷开 P95 和工作集回落；只有命中率与体验达标后，才考虑把低功耗路径预算从 8 MiB 继续下调或缩短 2 分钟租约。

### 11.5 本轮验证清单

- 前端定向测试、`npm run typecheck`、`npm run lint` 通过。
- 5.5 完整 Rust 测试曾验证通过；5.8/5.9 新增 Rust 定向测试和 `cargo fmt --check` 通过。
- 5.7 后执行生产前端构建，重型组件拆包生效；并完成详情页、播放队列的本地 UI 打开/关闭回归。
- 5.9 执行 50k/100k SQLite 合成基准并记录 P95、高水位和查询计划。
- 最终完整前端测试为 89 个测试文件、569 项测试全部通过；完整 Rust 测试为 119 项通过、2 项手动基准忽略，另有 31 项歌词集成测试通过；生产前端构建完成且拆包结构保持有效。
